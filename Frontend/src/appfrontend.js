...
const Reviews = () => {
  ...
  <div key={index}>
    <h2>{review.apt_name}</h2>
    <table>
      <tbody>
        <tr>
          <td>Average:</td>
          <td>{Math.round((review.facilities + review.happiness + review.safety + review.internet + review.location + review.management) / 6 * 10) / 10}</td>
        </tr>
        <tr>
          <td>Facilities:</td>
          <td>{review.facilities}</td>
        </tr>
        <tr>
          <td>Happiness:</td>
          <td>{review.happiness}</td>
        </tr>
        <tr>
          <td>Safety:</td>
          <td>{review.safety}</td>
        ...
        <button onClick={() => {
          chosenApartment = review.id;

          navigate(`/addapartment/:${review.id}`)
...
const UpdateReview = () => {
  ...
  const handleSubmit = (e) => {
    ...
    fetch(`http://127.0.0.1:4000/addapartment/${chosenApartment}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(formData),
  })
  ...
  <div>
      ...
      <form onSubmit={handleSubmit} className="login-form" style={{marginTop: '400px', backgroundColor: '#f8f9fa', padding: '20px', borderRadius: '10px'}}>
            ...
            <div style={{display: 'flex', alignItems: 'center', marginBottom: '10px'}}>
                ...
                <button type="submit" className="login-button" style={{backgroundColor: '#6c757d', color: 'white', padding: '10px 20px', borderRadius: '5px', marginTop: '20px'}}>Update</button>
        </form>
    </div>
  )
};
